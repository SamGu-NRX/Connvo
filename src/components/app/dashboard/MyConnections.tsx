...
// Update interface to use UserInfo instead of Profile
...
const ProfileList: React.FC<ProfileListProps> = ({
  ...
  // Sort profiles by online status and unread count
...
// Update ChatAppProps to use UserInfo
...
const ChatArea: React.FC<ChatAppProps> = ({
  ...
  const avatarColors = generateAvatarColor(activeProfile.name);

  // Determine if we should show gradient (no avatar or is bot)
...
const ModernChatApp: React.FC = () => {
  ...
  return (
    <div className="mx-auto flex w-full max-w-6xl gap-4 p-4">
      ...
      {/* User Profile Modal - Use user prop format expected by UserCard */}
