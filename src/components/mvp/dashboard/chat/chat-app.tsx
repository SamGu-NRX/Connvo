...
const ModernChatApp: React.FC = () => {
  ...
  const [activeProfile, setActiveProfile] = useState<UserInfo>(
    profiles[0] as UserInfo,
  );
  ...
  useEffect(() => {
    console.log("[Init] Initial activeProfile:", activeProfile);
  }, []);
  ...
  console.log(
    `[Render] showUserProfile: ${showUserProfile}, activeProfile ID: ${activeProfile?.id}`,
  );
  ...
  return (
    <div className="mx-auto w-full max-w-5xl p-4">
          ...
          <div className="flex-shrink-0 border-b p-4 dark:border-gray-800">
            <div className="flex items-center justify-between">
              <div className="flex min-w-0 items-center gap-3">
                ...
                <Button
                  variant="ghost"
                  className="h-auto flex-shrink-0 rounded-full p-0"
                  onClick={() => {
                    // --- DEBUG LOG 3: Log state on trigger click ---
                    console.log(
                      "[Trigger Click] Current activeProfile:",
                      activeProfile,
                    );
                    if (activeProfile) {
                      ...
                      console.error(
                        "[Trigger Click] Cannot open profile: activeProfile is null/undefined",
                      );
                    }
                  }}
                  aria-label={`View ${activeProfile?.name || "user"}'s profile`}
                >
                  {activeProfile && (
                    <Image
      ...
      <Dialog open={showUserProfile} onOpenChange={setShowUserProfile}>
        <DialogContent className="max-w-md bg-white p-0 dark:bg-gray-900">
          {/* --- DEBUG LOG 4: Log activeProfile inside DialogContent --- */}
          {(() => {
            console.log("[DialogContent Render] activeProfile:", activeProfile);
